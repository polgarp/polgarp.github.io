---
title: "e52 Assumptions draw the edge of the solution space, and agents draw it tighter"
categories:
  - Newsletter
tags:
  - Product discovery
...
Assumptions set the boundary of the problem space, and also define the region of potential in solution space. We learn about unsaid assumptions during testing solutions by asking questions about the problem space. Agents can help in exploring the solution space faster, but will also make the problem space more rigid.

{% include figure image_path="/assets/images/2026-08-23-Assumptions-draw-the-edge-of-the-solution-space.jpg" alt="An aerial view of a neighborhood in the fall" caption="Photo by [Patrick Federi](https://unsplash.com/@federi?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText) on [Unsplash](https://unsplash.com/photos/an-aerial-view-of-a-neighborhood-in-the-fall-JGkcj2xq5sc?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText)" %}
...
Holding the wrong beliefs, as in getting assumptions wrong is bad of course, since we and the product fail in one way or another. So we usually set out to both declare assumptions, and do some testing to manage these risks. We usually start optimistic, so at the beginning we mostly come up with reasons on why it will work.

In a good environment, we write with the product team the [list of assumptions]({% post_url 9am26/2023/2023-06-12-Writing-useful-hypotheses %}) we have, and how risky they are. If we are right about these the product will succeed. To account for limited time and resources the usual approach would be to construct tests and experiments to collect evidence about the riskiest assumptions. This is the right approach, and I've never regretted putting discipline to this.

But what about the things not on the list, we just took for granted?

Recognizing assumptions and collecting some data to make sure what we believe is correct is just one side of the puzzle, as **assumptions also bound the [solution space]({% post_url 9am26/2023/2023-06-05-Problem-space-solution-space %}), limiting what results are achievable**. 

Designers and more broadly product teams often fall into this trap, as they believe all problems will be addressed with their solution. Testing is of course right, as it tells us if the solution works and more broadly teaches something about the assumptions we stated. But it should also talk about the problem space and the deeper assumptions about whether a problem is worth solving. Product teams are especially bad at this, since we mostly believe we are already on the right track to solve the problem.

Considering a narrower problem space does remove risky solutions. And it also removes things the framing hid, cutting potentially more innovative ideas.

**We can't test things we haven't even considered**, if they were obscured by deeply held beliefs.

I call this dogma.

**Designers usually don't think about whether the solution should be a screen or not.** They just open Figma, and the version of the solution that might have been a policy change, or an email, or removing the step altogether, was just missing.

That's the difference. A belief is something someone said, which is what lets us argue with it, test it, and drop it when the data comes back unkind. Dogma never gets said, so there's nothing to argue with. It shows up in which options got sketched, which questions felt worth asking, and which version of the problem everyone recognised without needing to discuss it.

Can't we just think about our beliefs harder to also articulate dogma? It's not enough, as we need a different vantage point to look outside of the frame. 

**A longer list is still the same list.** And often we don't even start from our own frame, but one set up by the organizations and structures around us.

Testing the riskiest assumption is not enough, if the tests only address beliefs and don't touch dogma. Early in discovery there's no easy way to check if we work on the right problem, so good tests should talk about both problems and solutions.

This was the goal of making MVPs, not smaller versions of the product, but telling us whether the problem is worth solving. If a team tests only whether the solution works, it only asks the easier and less useful question.
...
My worry is that with agents, cheaper depth puts even higher cost to breadth. Running one more experiment against the opportunity we already like is nearly free now. Finding a different opportunity has the same cost as before.

But if you are building faster, learning is also faster, so the risks must be lower? 

Not really, since if we don't know why something failed, we won't know how to change course efficiently, even if we are faster. **Building more things from the same dogma will lead to the same region in the solution space**, we need to break out of that.
...
One way we arrive at dogma is the context we are working in, for example the tools we choose. Many designers have been starting their design work in Figma, which defaults to creating UI elements on screens. This bounds potential solutions into UI, often discounting other interaction paradigms or even alternative services. 
...
Even generating solutions with agents is bound by dogma. They generate average ideas or default solutions to the problem, which limits the potential of radically different solutions. 
...
In short, holding dogma doesn't necessarily result in bad solutions, but it is also less likely to produce great solutions. This is a major challenge with asking LLMs to assist in discovery.

There are two reasons for it. First, how people often work with agents is already defined by their own dogma. Second, LLMs answer based on the average of their training data.

Agents can be used to help with common assumptions, opening up new ideas, but returning to average also means the further away we are from that, the worse answers we get. It shows you the things you almost missed, but for things farther out, you'll need [people in the room]({% post_url 9am26/2026/2026-08-09-Discovery-artifacts-are-vacation-photos %}) to be able to articulate different assumptions, keeping in mind [groupthink]({% post_url 9am26/2025/2025-08-25-Combating-groupthink %}) problems. 
...
We need more purposeful argument points in the process to fix this.
...
Start with listing assumptions before you involve the agent, this sets a broader problem space, and also helps you to make better judgement on the agentic output.
...
Agents make very plausible artifacts, which also keeps us from thinking deeper about our assumptions. This includes also ready-made assumption lists generated by an agent, which we need solid ground to judge.

If you lead designers, testing is not the most valuable part to protect, since testing is getting cheaper now. It's the discussions leading to a test, where team members can ask whether this is the right problem at all. This is not slowing down, but making sure we are taking the right direction.
...
**Going faster doesn't mean we need less process. It means we now have to pay for the parts we used to get for free.**
